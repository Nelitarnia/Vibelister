...
export function initGridMouse(deps){
  const {
    ...
    // app callbacks
    isEditing, beginEdit, endEdit, render, ensureVisible,
    // view helpers
    viewDef, isModColumn, setModForSelection,
  } = deps;
  ...
  function onCellMouseDown(e){
    ...
    const t = findCellEl(e.target); if(!t) return;
    ...
    if (e.detail === 2) {
      if (!isModColumn(col)) {
        ...
        sel.r = r; sel.c = c; ensureVisible(r,c);
    ...
    if (!e.shiftKey && selection && selection.colsAll) selection.colsAll = false;
    ...
    if (e.shiftKey) {
      ...
      if (!clickInMulti) { // only collapse if click is outside the existing multi-selection
        selection.rows.clear();
        selection.rows.add(r);
        selection.anchor = r;
      }
    }
    sel.r = r; sel.c = c; ensureVisible(r,c); render();
  ...
  return () => {
    ...
    if (rowHdrs && onRowHdrWheel) rowHdrs.removeEventListener('wheel', onRowHdrWheel);
  };
}
