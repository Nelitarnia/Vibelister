...
export function initRowDrag(deps){
  ...
  function sheetYFromClient(y){ const r=sheet.getBoundingClientRect(); return y - r.top; }
  function contentYFromClient(y){ return sheetYFromClient(y) + sheet.scrollTop; }
  ...
  function moveDragLine(y){
    ...
    let target = clamp(base + deltaRows, 0, Math.max(0, nVisible - drag.blockLen));
  ...
  function onDragEnd(){
    ...
    if (autoScrollId != null) { cancelAnimationFrame(autoScrollId); autoScrollId = null; }
    dragLine.style.display='none';
    if(!drag) return;
    const didMove = !!moved;
    
    const arr=dataArray();
    if(!arr || arr.length===0){ drag=null; return }
    const start = Number.isFinite(drag.blockStart) ? drag.blockStart : clamp(drag.fromIndex,0,arr.length-1);
    ...
    if (!didMove) { drag=null; moved=false; return; }
    if (target >= start && target < start + len){ drag=null; moved=false; return; }
  ...
  rowHdrs.addEventListener('mousedown', (e) => {
    moved = false;
    const rh = e.target.closest('.rhdr'); if (!rh) return;
    const arr = dataArray();
    const idx = clamp(Number(rh.dataset.r) || 0, 0, Math.max(0, getRowCount() - 1));
    ...
    else if (typeof selection?.colsAll !== 'undefined') selection.colsAll = false;
  ...
  function startAutoScroll(){
    ...
    const tick = () => {
      if (!drag) { autoScrollId = null; return; }
      ...
      if (dy !== 0) {
        ...
        sheet.scrollTop = clamp(sheet.scrollTop + dy, 0, Math.max(0, maxScroll));
        onScrollDuringDrag();
      }
