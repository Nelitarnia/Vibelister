...
export function runUiTests(ui) {
  ...
  const deps = {
    ...
    SelectionNS: {
      ...
      extendTo: (r) => {
        const a = deps.selection.anchor ?? deps.sel.r;
        deps.selection.rows.clear();
        for (let i = Math.min(a, r); i <= Math.max(a, r); i++)
          deps.selection.rows.add(i);
        deps.selection.anchor = a;
      },
      setColsAll: (v) => {
  ...
  (function () {
    ...
    mousedown(c, 2);
    ok(
      began && began[0] === 3 && began[1] === 0,
      "dblclick begins edit on non-mod",
    );
  })();
