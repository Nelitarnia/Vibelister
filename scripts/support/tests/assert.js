...
export const section = (name) => (
  console.groupCollapsed(`%c${++SECTIONS}. ${name}`, "font-weight:bold"),
  () => console.groupEnd()
);
...
export const eq = (a, b, msg) =>
  ok(Object.is(a, b), msg ?? `eq: ${a} === ${b}`);
export const deepEq = (a, b, msg) =>
  ok(JSON.stringify(a) === JSON.stringify(b), msg ?? "deepEq");
...
export const done = () => {
  if (FAILS) throw new Error(`[tests] ${FAILS} failure(s)`);
