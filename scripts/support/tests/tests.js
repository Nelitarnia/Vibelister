...
export function runSelfTests(api) {
  ...
  const buildInteractionsPairs =
    api?.buildInteractionsPairs ?? g.buildInteractionsPairs;
  const rebuildActionColumnsFromModifiers =
    api?.rebuildActionColumnsFromModifiers ??
    g.rebuildActionColumnsFromModifiers;
  ...
  if (!model || !ensureSeedRows || !buildInteractionsPairs) {
    throw new Error(
      "[tests] Missing API: model/ensureSeedRows/buildInteractionsPairs",
    );
  }
  ...
  (function () {
    fresh();
    model.actions.splice(0);
    model.inputs.splice(0);
    addAction("A1");
    addAction("A2");
    addInput("I1");
    addInput("I2");
    addInput("I3");
    const { pairsCount, actionsCount, inputsCount } =
      buildInteractionsPairs(model);
    ok(
      actionsCount === 2 && inputsCount === 3 && pairsCount === 6,
      "basic A×I generation",
    );
  })();
  ...
  (function () {
    ...
    const tmp = model.modifiers[0];
    model.modifiers[0] = model.modifiers[1];
    model.modifiers[1] = tmp;
  ...
  (function () {
    ...
    ok(
      JSON.stringify(sigs) === JSON.stringify(expected),
      "variant ordering respects modifier row order",
    );
  })();
  ...
  (function () {
    ...
    ok(
      pairsCount === 3 * 1,
      "EXACT1 with {X,Y,Z} yields 3 variants; XY mutex is irrelevant under k=1",
    );
  })();
  ...
  (function () {
    ...
    if (!getIC || !setIC) {
      log("skip (no interactions helpers)");
      return;
    }
  ...
  (function () {
    const getS =
      api?.getStructuredCellInteractions ?? g.getStructuredCellInteractions;
    const applyS =
      api?.applyStructuredCellInteractions ?? g.applyStructuredCellInteractions;
    const setIC = api?.setInteractionsCell ?? g.setInteractionsCell;
    if (!getS || !applyS || !setIC) {
      log("skip (no interactions structured helpers)");
      return;
    }
    ...
    setIC(model, { textContent: "" }, vd, 0, 3, {
      endActionId: A.id,
      endVariantSig: "",
    });
    ...
    ok(
      pOutcome?.type === "outcome" && pOutcome.data?.outcomeId === O.id,
      "structured outcome payload",
    );
    ok(
      pEnd?.type === "end" && pEnd.data?.endActionId === A.id,
      "structured end payload",
    );
    ...
    buildInteractionsPairs(model);
    const ok1 = applyS(
      (r, c, v) => setIC(model, { textContent: "" }, vd, r, c, v),
      vd,
      1,
      2,
      pOutcome,
    ...
    // Verify display
    ok(
      (api?.getInteractionsCell ?? g.getInteractionsCell)(model, vd, 1, 2) ===
        "Hit",
      "pasted outcome displays",
    );
    ok(
      (api?.getInteractionsCell ?? g.getInteractionsCell)(model, vd, 1, 3) ===
        "Attack",
      "pasted end displays",
    );
  })();
  ...
  (function () {
    ...
    setIC(model, { textContent: "" }, vd, 0, 2, {
      endActionId: A.id,
      endVariantSig: `${m2.id}+${m1.id}`,
    });
    const txt = getIC(model, vd, 0, 2);
    ok(
      /Lift \\((Fall\\+Rise|Rise\\+Fall)\\)/.test(txt),
      "end display shows both mods in stable order",
    );
  })();
