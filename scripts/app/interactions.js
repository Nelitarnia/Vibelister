...
export function getInteractionsCell(model, viewDef, r, c) {
  ...
  if (keyL === "action" || keyL === "actionid" || keyL === "actionname") {
    ...
    const mods = sortedIds.map(id => model.modifiers.find(m=>m.id===id)?.name || "").filter(Boolean);
    return mods.length ? `${a.name || ""} (${mods.join("+")})` : (a.name || "");
  }

  // Right-hand identity column: Input (AI) or RHS Action (AA)
  	if (keyL === "input" || keyL === "inputid" || keyL === "rhsaction" || keyL === "rhsactionid") {
      const kind = pair && pair.kind ? String(pair.kind).toUpperCase() : 'AI';
  ...
  if (pk.field === "end") {
    if (typeof note.endActionId === "number") {
      ...
      if (note.endVariantSig) {
        const ids2 = String(note.endVariantSig).split("+").filter(Boolean).map(Number);
        const sortedIds2 = sortIdsByUserOrder(ids2, model);
        const modNames2 = sortedIds2.map(id => model.modifiers.find(m=>m.id===id)?.name || "").filter(Boolean);
...
export function getStructuredCellInteractions(model, viewDef, r, c) {
  const col = viewDef.columns[c]; if (!col) return null;
  const key = String(col.key || "");
  const pair = model.interactionsPairs[r]; if (!pair) return null;
  ...
  const pk = parsePhaseKey(key); if (!pk) return null;
  const k = noteKeyForPair(pair, pk.p);
  const note = model.notes[k]; if (!note) return null;

if (pk.field === "outcome") {
     if (note && note.outcomeId != null) {
       const oid = Number(note.outcomeId);
       if (Number.isFinite(oid)) return { type: "outcome", data: { outcomeId: oid } };
     }
  ...
  if (pk.field === "end") {
    if (typeof note.endActionId === "number") {
      return { type: "end", data: { endActionId: note.endActionId, endVariantSig: String(note.endVariantSig || "") } };
    }
    return null; // legacy free-text is readable in UI but never exported to clipboard
  }
    return null;
...
export function applyStructuredCellInteractions(setCell, viewDef, r, c, payload, modelFromCtx) {
...
const _incomingType = (payload && payload.type) ? String(payload.type).toLowerCase() : null;
const _incomingData = payload && payload.data ? payload.data : payload;

console.debug("[paste] colKey=", _colKey,
              "pk.field=", _pkField,
              "payload.type=", _incomingType,
              "payload.data keys=", _incomingData && Object.keys(_incomingData),
              "raw payload=", payload);
  ...
  if (!type || !data) {
    ...
    if (field === "outcome") {
      type = "outcome";
      data = (bare && typeof bare === "object" && "outcomeId" in bare) ? bare : { outcomeId: bare };
    } else if (field === "end") {
      if (bare && typeof bare === "object" && ("endActionId" in bare || "endVariantSig" in bare)) {
        type = "end"; data = bare;
      } else if (typeof bare === "number") {
        type = "end"; data = { endActionId: bare };
      } else {
  ...
  if (field === "end" && type === "action" && payload?.data && typeof payload.data.id === "number") {
    type = "end";
    data = {
  ...
  if ((field === "outcome" && type !== "outcome") || (field === "end" && type !== "end") || !data) {
    return false;
  }
  ...
  if (field === "outcome" && type === "outcome") {
    ...
    const evs = "endVariantSig" in (data || {}) ? String(data.endVariantSig || "") : "";
    wrote = !!setInteractionsCell(model, null, viewDef, r, c, { endActionId: eid, endVariantSig: evs });
  } else {
    return wrote;
  }
       ...
       if (pair && String(pair.kind).toUpperCase() === 'AA' && pk.p === 0) {
         ...
         if (val != null) {
           ...
           if (inv != null) {
             ...
             if (empty) {
               if (typeof inv === 'number') { mnote.outcomeId = inv; if ('result' in mnote) delete mnote.result; }
               else if (typeof inv === 'string') { mnote.result = inv; if ('outcomeId' in mnote) delete mnote.outcomeId; }
             }
           }
         }
       }
...
export function clearInteractionsCell(model, viewDef, r, c) {
  const col = viewDef.columns[c]; if (!col) return false;
  const key = String(col.key || "");
  const pair = model.interactionsPairs[r]; if (!pair) return false;

  if (key === "notes") {
    const k = noteKeyForPair(pair, undefined);
    const note = model.notes[k]; if (!note) return false;
    let changed = false;
    if ("notes" in note) { delete note.notes; changed = true; }
    if (!Object.keys(note).length) delete model.notes[k];
    return changed;
  }

  const pk = parsePhaseKey(key); if (!pk) return false;
  const k = noteKeyForPair(pair, pk.p);
  const note = model.notes[k]; if (!note) return false;
  let changed = false;

  if (pk.field === "outcome") {
    if ("outcomeId" in note) { delete note.outcomeId; changed = true; }
    if ("result" in note)    { delete note.result;    changed = true; }
  } else if (pk.field === "end") {
    if ("endActionId" in note)  { delete note.endActionId;  changed = true; }
    if ("endVariantSig" in note){ delete note.endVariantSig;changed = true; }
    if ("endFree" in note)      { delete note.endFree;      changed = true; }
  }
...
export function clearInteractionsSelection(model, viewDef, selection, sel, mode, status, render) {
	const rows = selection.rows.size ? Array.from(selection.rows).sort((a,b)=>a-b) : [sel.r];

	let cleared = 0;
	function clearField(note, key, pk){
		if (pk && pk.field === 'outcome') { if ('outcomeId' in note) { delete note.outcomeId; cleared++; } if ('result' in note) { delete note.result; cleared++; } return true; }
		if (pk && pk.field === 'end')     { if ('endActionId' in note) { delete note.endActionId; cleared++; } if ('endVariantSig' in note) { delete note.endVariantSig; cleared++; } if ('endFree' in note) { delete note.endFree; cleared++; } return true; }
		if (key === 'notes')              { if ('notes' in note) { delete note.notes; cleared++; } return true; }
		return false;
	}
	...
	if (mode === 'clearAllEditable') {
		...
		for (const r of rows) {
			...
			for (let c = 0; c < cols.length; c++) {
				...
				const pk = parsePhaseKey(key);
				if (!(key === 'notes' || (pk && (pk.field === 'outcome' || pk.field === 'end')))) continue;
