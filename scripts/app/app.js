...
function saveCurrentViewState(){
  const s = perViewState[activeView]; if (!s) return;
  s.row = sel.r|0; s.col = sel.c|0; s.scrollTop = sheet.scrollTop|0;
}
function restoreViewState(key){
  const s = perViewState[key]; if (!s) return { row:0, col:0, scrollTop:0 };
  return s;
}
...
function getColGeomFor(columns){
  ...
  if (_colGeomCache.key === key && _colGeomCache.widths && _colGeomCache.offs) return _colGeomCache;
...
document.addEventListener(
  ...
  (e) => {
    ...
    if (inSheet) return;
    try { if (paletteAPI?.isOpen?.()) return; } catch(_) {}
...
function setModForSelection(colIndex, target) {
	...
	for (const r of rows) {
		...
		setCellForKind('modTriState', kindCtx({ r, c: colIndex, col, row, v: next }), next);
	}
	render();
}
...
function pruneNotesToValidPairs() {
	...
	for (const p of (model.interactionsPairs || [])) {
		...
		} catch (_) { /* ignore malformed pairs while pruning */ }
	}
...
function sanitizeModifierRulesAfterDeletion(deletedIds) {
	...
	if (Array.isArray(model.modifierGroups)) {
		model.modifierGroups = model.modifierGroups
			...
			.filter((g) => {
				const ids = Array.isArray(g.ids) ? g.ids : Array.isArray(g.members) ? g.members : [];
				return ids.length >= 2;
			});
	}
		...
		model.modifierConstraints = model.modifierConstraints
				...
				for (const k of Object.keys(nc)) {
					...
					if (Array.isArray(v)) {
						...
						for (const kk of Object.keys(obj)) {
							...
							else if (Array.isArray(vv)) obj[kk] = vv.filter((x) => typeof x !== "number" || !del.has(x));
						}
						nc[k] = obj;
					}
				}
			...
			.filter((c) => {
				...
				for (const k of Object.keys(c)) {
					const v = c[k];
					if (Array.isArray(v) && v.filter((x) => typeof x === "number").length >= 2) return true;
				}
				return false;
			});
	}
...
function setCellSelectionAware(r, c, v) {
	if (activeView === 'interactions' && selection.rows && selection.rows.size > 1 && c === sel.c) {
...
if (!paletteAPI.openReference) {
  paletteAPI.openReference = ({ entity, target }) => {
    try {
      if (entity === 'outcome' && typeof paletteAPI.openOutcome === 'function') return paletteAPI.openOutcome(target);
      if (entity === 'action'  && typeof paletteAPI.openAction  === 'function') return paletteAPI.openAction(target);
      if (entity === 'input'   && typeof paletteAPI.openInput   === 'function') return paletteAPI.openInput(target);
    } catch (_) { /* noop */ }
...
const disposeMouse = initGridMouse({
  cellsLayer, rowHdrs, sheet, editor,
  sel, selection, SelectionNS,
  isEditing: () => editing,
  beginEdit, endEdit, render, ensureVisible,
  viewDef, isModColumn, dataArray, modIdFromKey, setCell, setModForSelection,
});
...
function render() {
  const s = perViewState[activeView]; if (s) { s.row = sel.r; s.col = sel.c; }
  if (activeView === 'interactions') {
  ...
  for (let r = vr.start; r <= vr.end; r++) {
    ...
    { const colsAll = (SelectionNS && SelectionNS.isAllCols && SelectionNS.isAllCols()) || !!selection.colsAll;
  ...
  for (let r = vr.start; r <= vr.end; r++) {
    ...
    for (let c = vc.start; c <= vc.end; c++) {
      ...
      if ((r % 2) === 1) d.classList.add('alt'); else d.classList.remove('alt');
      if (r === sel.r && c === sel.c) d.classList.add('selected'); else d.classList.remove('selected');
      d.style.opacity = '';
      if (activeView === 'interactions') {
        ...
        if (colKey) {
          ...
          if (s[0] === 'p' && i > 1) {
            ...
            if ((field === 'outcome' || field === 'end') && Number.isFinite(pNum)) {
...
function beginEdit(r, c) {
  ...
  if (activeView === "interactions") {
    ...
    if (res?.handled) { render(); return; }
    if (!res?.useEditor) { render(); return; }
  ...
  const left = offs[c] - sheet.scrollLeft,
	...
	if (activeView === 'interactions' && paletteAPI && paletteAPI.wantsToHandleCell && paletteAPI.wantsToHandleCell()) {
		paletteAPI.openForCurrentCell(left, r * ROW_HEIGHT - sheet.scrollTop, Math.max(200, w), editor.value || '');
...
function endEdit(commit = true) {
	...
	if (commit) {
		const rows =
			selection.rows.size > 1 ? Array.from(selection.rows).sort((a, b) => a - b) : [sel.r];
		for (const r of rows) {
			setCell(r, sel.c, val);
		}
	}
...
sheet.addEventListener("scroll", () => {
	...
	const s = perViewState[activeView]; if (s) s.scrollTop = sheet.scrollTop|0;
...
function setActiveView(key) {
	...
	if (key === "interactions") { rebuildInteractionsInPlace(); rebuildInteractionPhaseColumns(); }
	if (tabActions) {
	...
	if (tabInteractions) {
		...
		tabInteractions.setAttribute("aria-selected", String(key === "interactions"));
	}
...
selection.rows.clear(); selection.rows.add(sel.r); selection.anchor = sel.r;
...
if (tabInteractions) tabInteractions.onclick = () => setActiveView("interactions");
...
function updateProjectNameWidget() {
	...
	if (!w) return;
	if (w.value !== (model.meta?.projectName || "")) w.value = model.meta?.projectName || "";
	if (!w._bound) {
...
function upgradeModelInPlace(o) {
	...
	if (!('interactionsMode' in o.meta) || (o.meta.interactionsMode !== 'AI' && o.meta.interactionsMode !== 'AA')) {
...
function runSelfTests() {
  ...
  Promise.all([
    ...
    .then(([v, inter, m, uiTests, ui]) => {
      ...
      try {
        const runUi = (uiTests && (uiTests.runUiTests || uiTests.default)) || null;
...
const MenusNS = { closeAllMenus: menusAPI.closeAllMenus, updateViewMenuRadios: menusAPI.updateViewMenuRadios };
