	...
	function render() {
		...
		groups.forEach((g, gi) => {
			...
			const req = h("label", null, [
				h("input", { type: "checkbox", checked: g.required ? "checked" : null }),
			...
			function num(label, prop) {
				...
				return h(
					"label",
					{ style: "display:inline-flex;gap:6px;align-items:center;margin-left:6px;" },
					[label, inp],
				);
			}
		...
		const list = h(
			...
			cons.map((c, ci) => {
				let txt = "";
				if (c.type === "REQUIRES") txt = `${nameOf(c.a)} requires ${nameOf(c.b)}`;
				else if (c.type === "FORBIDS") txt = `${nameOf(c.a)} forbids ${nameOf(c.b)}`;
				else if (c.type === "MUTEX") txt = `Mutex: ${c.ids.map(nameOf).join(", ")}`;
				const row = h(
					"div",
					{
						style:
		...
		saveBtn.onclick = () => {
			...
			const validIds = new Set((model.modifiers || []).map(m => m.id).filter(Number.isFinite));
			const uniqNums = (arr) => Array.from(new Set((arr || []).map(Number).filter(n => Number.isFinite(n))));

			// --- Groups: sanitize ---
			let newGroups = (groups || []).map((g) => {
				const memberIds = uniqNums(g.memberIds).filter(id => validIds.has(id)).sort((a,b)=>a-b);
				...
				if (mode === "RANGE") {
					let kMin = Number(g.kMin), kMax = Number(g.kMax);
					if (!Number.isFinite(kMin)) kMin = undefined;
					if (!Number.isFinite(kMax)) kMax = undefined;
					if (kMin !== undefined && kMax !== undefined && kMin > kMax) [kMin, kMax] = [kMax, kMin];
					ng.kMin = kMin; ng.kMax = kMax;
				}
				return ng;
			});
			// Drop empty/singleton groups (no semantic weight)
			newGroups = newGroups.filter(g => Array.isArray(g.memberIds) && g.memberIds.length >= 2);

			// --- Constraints: sanitize ---
			let newCons = (cons || []).map((c) => {
				if (c.type === "MUTEX") {
					const ids = uniqNums(c.ids).filter(id => validIds.has(id)).sort((a,b)=>a-b);
					return ids.length >= 2 ? { type: "MUTEX", ids } : null;
				}
				if (c.type === "REQUIRES" || c.type === "FORBIDS") {
					const a = Number(c.a), b = Number(c.b);
					if (Number.isFinite(a) && Number.isFinite(b) && a !== b && validIds.has(a) && validIds.has(b)) {
