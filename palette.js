...
export function initPalette(ctx){
  const {
    editor, sheet,
    getActiveView, viewDef, sel,
    model, setCell, render,
    HEADER_HEIGHT,
    endEdit,
  } = ctx;
  ...
  const sortIdsByUserOrder = (ids, model) => {
    ...
    return ids.slice().sort((x,y) => (order.get(x) ?? 1e9) - (order.get(y) ?? 1e9));
  };
  const idsFromSig = (sig) => String(sig || '').split('+').filter(Boolean).map(Number).filter(Number.isFinite);

  // Normalize “Action (A+B)” or “Action — A+B” to “Action +A +B”
  const normalizeCellTextToQuery = (s) => {
    const txt = String(s || '').trim();
    if (!txt) return '';
    let action = txt, modsPart = '';
    const mParen = txt.match(/^([^()—]+)\s*\(([^)]+)\)\s*$/);
    if (mParen) { action = mParen[1].trim(); modsPart = mParen[2].trim(); }
    else {
      const mDash = txt.match(/^([^()—]+)\s*—\s*(.+)$/);
      if (mDash) { action = mDash[1].trim(); modsPart = mDash[2].trim(); }
    }
    let query = action;
    if (modsPart) {
      const parts = modsPart.split(/[+,\s]+/).map(x => x.trim()).filter(Boolean);
  ...
  const MODE_MAP = [
    {
      ...
      makeItems: (model, parsed) => {
        const q = parsed.q || '';
        const rows = (model.outcomes || []).filter(r => (r.name || '').trim());
        const out = [];
        for (const r of rows){
          const nm = r.name || '';
      ...
      commit: (it) => { setCell(sel.r, sel.c, Number(it.data.outcomeId)); render(); },
    ...
    },
    {
      ...
      parseQuery: (raw) => {
        ...
        const mods = [], a = [];
        for (let t of tokens){
          if (t.startsWith('+') && t.length > 1) mods.push(t.slice(1).toLowerCase());
      ...
      makeItems: (model, parsed) => {
        const { a, mods } = parsed;
        const actions = (model.actions || []).filter(x => (x.name || '').trim());
        const pairs = Array.isArray(model.interactionsPairs) ? model.interactionsPairs : [];
        const seen = new Set();
        const out = [];
        ...
        if (pairs.length){
          for (let i=0;i<pairs.length;i++){
            ...
            const modIds = sortIdsByUserOrder(idsFromSig(p.variantSig || ''), model);
            const modNames = modIds.map(id => model.modifiers.find(m=>m.id===id)?.name || '').filter(Boolean);

            if (mods.length){
              const lowerMods = modNames.map(s=>s.toLowerCase());
              if (!mods.every(tok => lowerMods.some(nm => nm.includes(tok)))) continue;
            }
            const display = modNames.length ? `${actionName} — ${modNames.join('+')}` : actionName;
            out.push({
              display,
              data: { endActionId: act.id, endVariantSig: modIds.length ? modIds.join('+') : '' },
            });
          }
        } else {
          ...
          for (const aRow of actions){
            ...
            out.push({ display: nm, data: { endActionId: aRow.id, endVariantSig: '' }});
          }
        }
        return out;
      },
      commit: (it) => { setCell(sel.r, sel.c, it.data); render(); },
      recentKeyOf: (it) => `a:${it.data.endActionId}|${it.data.endVariantSig || ''}`,
    },
  ];
  ...
  const pal = {
    el: null, listEl: null,
    isOpen: false, items: [], selIndex: -1,
    left: 0, top: 0, width: 0,
    mode: null, query: '', lockHoverUntil: 0,
    // MRU per mode
  ...
  function openForCurrentCell(left, top, width, initialText=''){
    ...
    if (mode.consumeTyping) { try{ editor.value=''; }catch(_){} }
  ...
  function close(){
    ...
    pal.isOpen = false;
    pal.items = []; pal.selIndex = -1; pal.query = '';
    pal.mode = null;
  }

  function refilter(){
    const mode = pal.mode; if (!mode) return;
  ...
  function updateRecent(item){
    const mode = pal.mode; if (!mode || !mode.recentKeyOf) return;
    const key = mode.recentKeyOf(item); if (!key) return;
    const bag = pal.recent.get(mode.name) || [];
  ...
  function renderList(isRecent=false){
    ...
    if (!pal.items.length){
      ...
      d.onmousedown = (ev) => { ev.preventDefault(); ev.stopPropagation(); };
      d.onclick = (ev) => { ev.preventDefault(); ev.stopPropagation(); close(); endEdit(false); };
      frag.appendChild(d);
    } else {
      if (isRecent){
      ...
      pal.items.forEach((it, idx) => {
        ...
        item.onmouseenter = () => {
          ...
          pal.selIndex = idx; renderList(isRecent);
        };
        const commitClick = (e) => { e.preventDefault(); pick(idx); };
  ...
  function pick(idx){
    ...
    const it = pal.items[idx]; if (!it) return;
    pal.mode.commit(it);
    updateRecent(it);
    close();
    endEdit(false);
  ...
  const onEditorKeyDown = (e) => {
    ...
    if (e.key === 'Escape') {
      ...
      close(); endEdit(false); return;
    }
    if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
      e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation?.();
      const n = pal.items.length; if (!n) return;
    ...
    if (e.key === 'Enter') {
      ...
      else { close(); endEdit(false); }
      return;
    }

    // Outcome’s classic behavior: synthesize query from keystrokes even if editor is empty
    if (mode.consumeTyping) {
      if (e.key === 'Backspace') { e.preventDefault(); pal.query = pal.query.slice(0, -1); refilter(); return; }
      if (e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
        e.preventDefault(); pal.query += e.key; refilter(); return;
      }
    }
  };
  ...
  // Close on scroll
  sheet.addEventListener('scroll', () => { if (pal.isOpen){ close(); endEdit(false); } }, { passive: true });
